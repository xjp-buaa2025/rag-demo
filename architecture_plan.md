...
## 1. 架构目标
搭建一个轻量、本地化、高可扩展的 RAG (Retrieval-Augmented Generation) 项目，实现本地/远程大模型（OpenAI 兼容 API）检索本地数据来完成特定问答任务。项目架构需支持未来平滑接入 LangChain 与各类 Skills 功能。
...
- **向量与存储层 (Storage Layer)**
  ...
  - 实现：使用 Qdrant 作为向量数据库，支持文本向量与图像向量双通道检索。
- **检索与召回层 (Retrieval Layer)**
  - 职责：查询向量化转化并进行最近邻检索，支持在未来扩展全文+向量混合检索及 Reranker（重排）功能。
- **大模型生成层 (LLM Layer)**
  - 职责：管理与配置对大模型的请求调用。
  - 实现：通过标准兼容 OpenAI 的 API 对接主模型与备份模型（MiniMax / SiliconFlow），支持运行时降级。
...
- [ ] **Phase 2: 构建文档管道 (Ingest)** 
  ...
  - [ ] 连接 Qdrant 完成向量数据入库存储 (`main_ingest.py`)
- [ ] **Phase 3: 构建检索与生成 (Chat)** 
  - [ ] 配置 OpenAI 兼容 LLM 服务接入（主备模型）
  - [ ] 结合本地召回文本和 RAGFlow Prompt，调试最终问答效果（`run_backend.py` + `/chat`）
